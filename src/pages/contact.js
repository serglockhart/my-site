  ...
  $contactForm.submit(function handleSubmit(event) {
    ...
    $.post("/contact", str, function handleResponse(data) {
      if (!data.success) {
        ...
        $contactForm.html(
          '<div class="modal-body"><div class="alert alert-success marg-bot-0">Your email was sent successfully! I will get back to you as quickly as possible.</div></div><div class="modal-footer"><button type="button" class="btn btn-secondary" data-dismiss="modal">Close</button></div>'
        )
      ...
      $message.html(
        '<div class="alert alert-danger">Something went wrong. Check the form and try again.</div>'
      )
...
// 					// If sending the message was unsuccessful
// 					$contactMessage.html(
// 						`<div class="alert alert-danger marg-bot-1">${data.error}</div>`
// 					);
...
// 					$contactPageForm.html(
// 						'<div><div class="alert alert-success marg-bot-1">Your email was sent successfully! I will get back to you as quickly as possible.</div><a href="/" class="btn btn-primary">Back to home</a></div>'
// 					);
// 				}
// 			}).fail(function handleFailure() {
// 				// If there was some other error
// 				$contactMessage.html(
// 					'<div class="alert alert-danger marg-bot-1">Something went wrong. Check the form and try again.</div>'
// 				);
